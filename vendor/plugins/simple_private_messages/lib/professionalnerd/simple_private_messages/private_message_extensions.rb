      ...
      module ActMethods
        ...
        def is_private_message(options = {})
          ...
          unless included_modules.include? InstanceMethods
            ...
            if options[:sender_class_name]
              belongs_to :sender,
                       :class_name => options[:sender_class_name],
                       :foreign_key => 'sender_id'
            end

            if options[:receiver_class_name]
              belongs_to :recipient,
                       :class_name => options[:receiver_class_name],
                       :foreign_key => 'recipient_id'
            end
            
            if !options[:sender_class_name] && !options[:receiver_class_name]
              belongs_to :sender,
