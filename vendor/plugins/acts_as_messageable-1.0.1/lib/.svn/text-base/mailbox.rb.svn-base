    ...
    module Messageable #:nodoc
      class Mailbox
        ...
        def mail_count(filter = :all, options = {})
          default_options = {:conditions => ["mail.doctor_id = ?", @doctor.id]}
        ...
        def add(msg)
          ...
          attributes[:read] = msg.sender.id == @doctor.id
          mail_msg = Mail.new(attributes)
          @doctor.mail << mail_msg
          return mail_msg
        end
        ...
        def mark_as_unread(options = {})
          default_options = {:conditions => ["mail.mailbox != ?",@doctor.mailbox_types[:sent].to_s]}
          add_mailbox_condition!(default_options, @type)
        ...
        def move_to(mailbox, options = {})
          ...
          trash = mailbox == @doctor.mailbox_types[:deleted].to_sym
          default_options = {}
          add_mailbox_condition!(default_options, @type)
          if(!trash)
            ...
            CASE mail.mailbox
              WHEN '#{@doctor.mailbox_types[:sent].to_s}' THEN mail.mailbox
              ELSE '#{mailbox.to_s}'
        ...
        def empty_trash(options = {})
          default_options = {:conditions => ["mail.doctor_id = ? AND mail.trashed = ?", @doctor.id, true]}
          add_mailbox_condition!(default_options, @type)
          return delete_mail(default_options, options)
        end
        ...
        def get_mail(default_options, options)
          ...
          return @doctor.mail.find(:all, default_options)
        end
        def update_mail(updates, default_options, options)
          build_options(default_options, options) unless options.empty?
          return @doctor.mail.update_all(updates, default_options[:conditions])
        end
        ...
        def add_mailbox_condition!(options, mailbox_type)
          ...
          return add_conditions!(options, "mail.mailbox = ? AND mail.trashed = ?", mailbox_type.to_s, false) unless mailbox_type == @doctor.mailbox_types[:deleted]
